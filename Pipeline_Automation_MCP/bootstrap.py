#!/usr/bin/env python3
"""
Unity Pipeline Automation MCP plugin bootstrap (SessionStart hook entry point)
==============================================================================
...
Pipeline Automation MCP server's venv exists in the plugin's persistent data directory
...
background and to surface Unity sign-in status (the standard
Claude Code /mcp OAuth flow only exists for remote HTTP servers, not
local stdio servers like this one).
...
_spec = importlib.util.spec_from_file_location(
    "uap_install_pipeline_automation", PLUGIN_ROOT / "install.py"
)
install = importlib.util.module_from_spec(_spec)
sys.modules["uap_install_pipeline_automation"] = install
_spec.loader.exec_module(install)

# A build lock older than this is considered crashed/stale and is replaced.
LOCK_STALE_SECONDS = 6 * 60 * 60

CONTEXT_PREFIX = "Unity Pipeline Automation MCP:"
...
def auth_context_line() -> str | None:
    if signed_in():
        return None
    return (
        f"{CONTEXT_PREFIX} not signed in to Unity. Pipeline Automation tools "
        f"will not work until the user signs in — before calling them, ask "
        f"the user, then run the unity_login tool (it opens a browser "
        f"sign-in). One sign-in also covers the Unity Asset Manager plugin."
    )
...
def hook_main() -> int:
    ...
    if lock_is_active():
        print(
            ...
            f"in the background. When it finishes, open /mcp and RECONNECT the "
            f"pipeline_automation server — that is what brings it up; /reload-plugins does "
            f"not reliably start an MCP server. No restart needed. "
    ...
    print(
        ...
        f"background (first run or plugin update). The pipeline_automation MCP "
