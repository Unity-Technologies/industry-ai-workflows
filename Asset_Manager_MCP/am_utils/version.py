"""
Runtime identity for the Unity Asset Manager MCP server
=======================================================
...
request — X-Unity-Cloud-Api-Source ("uam_mcp@<version>", Unity's
...
# "unknown", so the attribution headers would degrade to uam_mcp@unknown with
...
def user_agent() -> str | None:
    ...
    Unset env -> computed "UAM_MCP/<version> (<agent>; <OS>)"; env set
...
def api_source() -> str:
    """The api-source header value, "uam_mcp@<version>" — Unity's
    "[source]@[version]" analytics-headers format (a broken manifest yields
    "uam_mcp@unknown", matching the User-Agent fallback)."""
...
def attribution_headers() -> dict[str, str]:
    ...
    ("uam_mcp@<version>"; captured by the gateway logs for attribution) plus
