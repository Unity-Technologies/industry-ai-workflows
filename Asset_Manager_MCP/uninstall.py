#!/usr/bin/env python3
"""
Unity Asset Manager MCP uninstaller
===================================
...
_spec = importlib.util.spec_from_file_location(
    "uap_install_asset_manager", Path(__file__).resolve().parent / "install.py"
)
_install = importlib.util.module_from_spec(_spec)
sys.modules["uap_install_asset_manager"] = _install
...
SIBLING_DATA_IDS = (
    "upa-mcp-industry-ai-workflows",
    "upa-mcp-inline",
)
SIBLING_DISPLAY = "Unity Pipeline Automation"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Uninstall the Unity Asset Manager MCP server venv and env files."
    )
    parser.add_argument(
    ...
    print(f"Unity Asset Manager MCP uninstaller | plugin root: {REPO_ROOT}")
