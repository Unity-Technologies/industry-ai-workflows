#!/usr/bin/env python3
"""
Unity Asset Manager MCP plugin bootstrap (SessionStart hook entry point)
========================================================================
...
Asset Manager MCP server's venv exists in the plugin's persistent data directory
...
_spec = importlib.util.spec_from_file_location(
    "uap_install_asset_manager", PLUGIN_ROOT / "install.py"
)
install = importlib.util.module_from_spec(_spec)
sys.modules["uap_install_asset_manager"] = install
_spec.loader.exec_module(install)

# A build lock older than this is considered crashed/stale and is replaced.
LOCK_STALE_SECONDS = 6 * 60 * 60

CONTEXT_PREFIX = "Unity Asset Manager MCP:"
...
def auth_context_line() -> str | None:
    if signed_in():
        return None
    return (
        f"{CONTEXT_PREFIX} not signed in to Unity. Asset Manager tools will "
        f"not work until the user signs in — before calling them, ask the "
        f"user, then run the unity_login tool (it opens a browser sign-in). "
        f"One sign-in also covers the Unity Pipeline Automation plugin."
    )
...
def hook_main() -> int:
    ...
    if lock_is_active():
        print(
            ...
            f"asset_manager server — that is what brings it up; /reload-plugins does "
    ...
    print(
        ...
        f"background (first run or plugin update). The asset_manager MCP "
