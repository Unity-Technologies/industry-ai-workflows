#!/usr/bin/env python3
"""
Unity Asset Transformer MCP installer
=====================================

Builds the venv for the Asset Transformer MCP server: creates it and pip-installs this
server's dependencies from its hash-pinned requirements.lock.
The Asset Transformer SDK (`pxz`) is installed from Unity's
customer-accessible Pixyz package index automatically.
...
Notes
-----
- This server requires EXACTLY Python 3.12: its dependencies are a full
  pin set, because the venv also hosts the pxz SDK whose own declared pins
  conflict with them. If the interpreter running this script is not 3.12,
  one is auto-discovered (`py -3.12`, `python3.12`, ...); pass
  `--python <path-to-python3.12>` to pick one explicitly.

- This server does NOT use Unity sign-in. It authenticates against a
  Pixyz FlexLM license server instead; see AT_LICENSE_SERVER_HOST.
...
PLUGIN_DATA_IDS = ("uat-mcp-industry-ai-workflows", "uat-mcp-inline")
...
SPECS: list[McpSpec] = [
    McpSpec(
        ...
        # Mirrors this plugin's userConfig. AT_LICENSE_TOKENS,
        # AT_LICENSE_FAIL_FAST and AT_MAX_IMPORT_BYTES are deliberately absent:
        # the server still reads them from the environment, they are just not
        # worth prompting every user for (see .env.example).
        user_config_env=(
...
def resolve_base_python(cli_python: str | None) -> str:
    ...
    Asset Transformer's venv is built with Python 3.12 (see REQUIRED_PYTHON).
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Install the Unity Asset Transformer MCP server venv.")
    parser.add_argument(
        "--python",
        default=None,
    ...
    print(f"Unity Asset Transformer MCP installer | plugin root: {PLUGIN_ROOT}")
