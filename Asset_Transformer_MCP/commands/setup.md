---
description: Build or repair this plugin's Python environment. Use when the asset_transformer MCP server is missing, failed to start, or its tools are unavailable — typically because the plugin was installed part-way through a session, so the SessionStart hook that normally builds it never ran.
---

# Set up the Unity Asset Transformer MCP server
...
1. Find this plugin's root — the directory containing `install.py`, alongside
   `at_mcp_server.py`. It is the installed plugin's cache directory, normally
   `~/.claude/plugins/cache/industry-ai-workflows/uat-mcp/<version>/`.
...
2. Run that plugin's own installer, with `CLAUDE_PLUGIN_DATA` pointing at its
   ...
   ```bash
   CLAUDE_PLUGIN_DATA="$HOME/.claude/plugins/data/uat-mcp-industry-ai-workflows"      python "<plugin root>/install.py" --non-interactive
   ```

   On Windows PowerShell:

   ```powershell
   $env:CLAUDE_PLUGIN_DATA = "$HOME\.claude\plugins\data\uat-mcp-industry-ai-workflows"
   python "<plugin root>\install.py" --non-interactive
   ```


This one downloads the Pixyz SDK and can take several minutes. That is normal — do not kill it.

3. Confirm it finished with `AT: OK` and that it wrote `stamp-AT.json`.
...
- Do not install into a different directory. The `command` in this plugin's
  `mcp-servers.json` points at `$CLAUDE_PLUGIN_DATA/venvs/AT`, so an
